...
<img src="img/deseq_workflow_fullnograyedout.png" width="400">
...
Modeling is a mathematically formalized way to approximate how the data behaves given a set of parameters (i.e. size factor, dispersion). DESeq2 will use this formula as our model for each gene, to fit the normalized count data to it. After the model is fit, coefficients (**log2 foldchanges or LFC**) are estimated for each sample group along with their standard error using the formula:

<img src="img/NB_model_formula_betas.png" width="600">

The model for each gene for a design like ours with three levels, can be simplified to:

$$\LARGE y = \beta_0 + \beta_1 x_{B} + \beta_2 x_{C}$$

Where:

* $y$ is the fitted counts for each gene

* $\beta_0$ is the average of the reference group (in this example “A”)

* $\beta_1$ is the difference between “B” and the reference group “A”

* $\beta_2$ is the difference between “C” and the reference group “A”.

* $x_B$indicates whether a sample belongs to group B or not.

* $x_C$ indicates whether a sample belongs to group C or not.

The $\beta$ coefficents are the estimates for the LFC for each sample group. However, when counts are low, these LFC are inherently noisier due to the large dispersion we observe with low read counts. To avoid this, the LFCs calculated by the model need to be adjusted**.
