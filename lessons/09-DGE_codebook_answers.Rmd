...
geomeanLog <- function(x, na.rm = TRUE){
...
Remember the steps:
1. take the geometric mean of each row
2. divide each row by the geometric mean
3. take the median of each column to get the size factors for each sample; remember you only want the median of the finite values
4. divide each column by its size factor
...
# "sweeps" across columns and divides each each by its size factor
# sweep(x, MARGIN, STATS, FUN = "-", check.margin = TRUE)
# where STATS is a summary statistic to be 'swept' out
# where MARGIN = 1 to apply the function to rows of a matrix
# and MARGIN = 2 to apply the function to columns of a matrix
# where check.margin checks that the length of STATS matches the 
# length of the dimension you are operating on, e.g. for a function # acting on the columns of a matrix, the length of STATS should be 
# equal to ncol(matrix)
# here sweep 'sweeps' across each row and divides it by its 
# geometric mean

div  <- sweep(data,1,geo,FUN="/")
...
- Ans: color, fontsize_row,border_color
...
- Ans: sampletype, you could also use MOVexpr, but they are redundant and sampletype is more descriptive.
...
plot(colSums(counts(dds, normalized = FALSE)),pch=19, type = "b", col = "darkorange")
points(colSums(counts(dds, normalized = TRUE)),pch=19, type = "b", col = "blue")


colSums(counts(dds, normalized=T))/mean(colSums(counts(dds, normalized=T)))

colSums(data)/colSums(counts(dds,normalized=TRUE))

sizeFactors(dds)

```

- Ans: they are quite different

**How does the unnormalized `colSums` of the counts divided by the `mean` of these counts compare to the `sizeFactors`?**
```{r}

plot(colSums(data)/median(colSums(data)),pch=19, type = "b", col = "darkorange")

points(sizeFactors(dds), pch=19, type = "b", col = "blue")
```

- Ans: They are almost exactly the same
...
mn <- apply(norm.cts,1,mean)
mn=mn[order(mn)]
mn <- mn[mn!=0]


v=apply(norm.cts,1,var)
...
In the most recent versions of DESeq2, the shrinkage of LFC estimates is **not performed by default**. This means that the log2 foldchanges would be nearly the same as those calculated by:
```{r}

normalized_counts <- counts(dds,normalized = T)

normalized_counts_group <- apply(normalized_counts,1, function(x) x=tapply(x,meta$sampletype,mean))

normalized_counts_group <- t(normalized_counts_group)

normalized_counts_group1 <- normalized_counts_group[,1]

normalized_counts_group3 <- normalized_counts_group[,3]

oe=log2 (normalized_counts_group3 / normalized_counts_group1)
```

Let's look at the log2 fold change for the gene "MOV10":

```{r}
oe["MOV10"]
```

We can compare this to the log2 fold change calculated by the `DESeq` function like this:
...
They are nearly the same, the difference is due to the `DESeq` function estimating the counts using by fitting to a negative binomial model, taking the dispersion effect into account.
