...
Hint: use the `apply` function; and you can combine it with the `sweep` function but you don't need to
...
mcols(dds)["MOV10",
           "sampletype_MOV10_overexpression_vs_control"]

```

They are nearly the same, the difference is due to the `DESeq` function estimating the counts using by fitting to a negative binomial model first and taking the dispersion effect into account.
